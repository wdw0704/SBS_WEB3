...
$(function() {
    ...
    // 현재 슬라이드 위치/인덱스
    let currentNo = 1
    let currentIndex = 0                // 왼쪽으로부터 떨어진 위치 (left)
    ...
    //          -= ( 400 * 2)
    // 현재 위치 -= -800px
    currentPositon -= slideWidth * initNum
    list.css( {left : currentPositon})
    ...
    $('.prev').on('click', ()=> {
        ...
        currentPositon += slideWidth
        list.stop().animate({ left : currentPositon}, 1000)
    ...
    $('.dot').on('click', function() {
        ...
        // (이전)
        if( gap < 0 ) {
            currentPositon -= (slideWidth * absGap)
            list.css( {left : currentPositon})

            for (let i = 0; i < absGap; i++) {
                $('.slide-item:last-child').prependTo
            }

            currentPositon += (slideWidth*absGap)
            list.animate({ lsft : currentPositon}, 1000)
        }
